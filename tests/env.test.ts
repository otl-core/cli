...
describe("env utilities", () => {
  it("reads configuration from .env.local file", async () => {
    const envContent = [
      "SITE_ID=test-deploy-123",
      "SITE_ACCESS_TOKEN=otl_dpl_test_token",
    ...
    expect(config.siteId).toBe("test-deploy-123");
  ...
  it("falls back to process.env when no .env.local exists", async () => {
    process.env["SITE_ID"] = "env-deploy";
    process.env["SITE_ACCESS_TOKEN"] = "env-token";
    process.env["API_URL"] = "https://env-api.com";

    const loadConfig = await importLoadConfig();
    const config = loadConfig();

    expect(config.siteId).toBe("env-deploy");
  ...
  it("exits with error when SITE_ID is missing", async () => {
    const envContent = "SITE_ACCESS_TOKEN=test-token\n";
    fs.writeFileSync(path.join(tmpDir, ".env.local"), envContent, "utf-8");
    ...
    expect(mockError).toHaveBeenCalledWith(
      expect.stringContaining("Missing SITE_ID"),
    );
  });

  it("exits with error when SITE_ACCESS_TOKEN is missing", async () => {
    const envContent = "SITE_ID=test-deploy\n";
    fs.writeFileSync(path.join(tmpDir, ".env.local"), envContent, "utf-8");
  ...
  it(".env.local values take precedence over process.env", async () => {
    process.env["SITE_ID"] = "from-env";
    process.env["SITE_ACCESS_TOKEN"] = "from-env-token";

    const envContent = [
      "SITE_ID=from-file",
      "SITE_ACCESS_TOKEN=from-file-token",
    ].join("\n");
    fs.writeFileSync(path.join(tmpDir, ".env.local"), envContent, "utf-8");
    ...
    expect(config.siteId).toBe("from-file");
