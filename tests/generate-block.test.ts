...
describe("generateBlockComponent", () => {
  ...
  it("uses the correct component name from kebab-case type", () => {
    const output = generateBlockComponent("entry-list", [
      field({ id: "limit", type: "number" }),
    ]);

    expect(output).toContain("interface EntryListConfig");
    expect(output).toContain("export function EntryListBlock");
    expect(output).toContain("BlockComponentProps<EntryListConfig>");
  });
});
